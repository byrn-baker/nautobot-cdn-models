...
class CdnGitRepository(PrimaryModel):
    ...
    secrets_group = models.ForeignKey(
        ...
        blank=True,
        null=True,
        related_name="cdn_git_repositories",
    )
    ...
    def clean(self):
        ...
        if self.provided_contents:
            ...
            duplicate_repos = CdnGitRepository.objects.filter(remote_url=self.remote_url).exclude(id=self.id).filter(q)
    ...
    def get_latest_sync(self):
        ...
        # This will match all "CdnGitRepository" jobs (pull/refresh, dry-run, etc.)
